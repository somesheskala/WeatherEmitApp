...
object WeatherDatasetApp {
  ...
  val conf = new SparkConf().setAppName("WeatherEmitApp").setMaster("local[2]")
  val sc = new SparkContext(conf)
  def main(args: Array[String]) {
    val isEmailEnabled = sc.getConf.get("spark.isEmailEnabled");
    try{
        LOG.info("WeatherEmitApp is started")
       ...
       val objDataBean = sc.textFile("file:"+inputFilepath)
        ...
        val finalDF = objDataBeanDF.map(x => weatherBeanOut(x.getString(1), x.getDouble(3)+","+x.getDouble(4)+","+x.getDouble(2), x.getString(5), getScopes(x.getDouble(6),x.getDouble(7), x.getInt(8)),x.getDouble(6),x.getDouble(7), x.getInt(8))).toDF()
...
private def getScopes(temp: Double,pre: Double,hum:Int): String = {
    var conditio="";
